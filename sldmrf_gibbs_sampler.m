function [labels,psi,theta]=sldmrf_gibbs_sampler(documents,connections,K,V)
%-------------------------------------------------------------
...
%--------------------------------------------------------------------------
%    seek the gravity center as the center for Guassian mean of part
%--------------------------------------------------------------------------
sum_dict=zeros(V,1);                     
for m=1:M
    for n=1:N_m(m)
        w=documents{m,1}(n);
        sum_dict(w)=sum_dict(w)+1;
    end;
end;
...
for i=1:V
    if sum_dict(i)>0
        ...
        sum_dict_position=[sum_dict_position; repmat([word_x,word_y,word_z],num_word,1)];
    end;
end;

mu_0=mean(sum_dict_position);
sigma_0=K*cov(sum_dict_position);
std_0=std(sum_dict_position);
Gaussian_means=zeros(K,3); 
Gaussian_cov=0.5*max(std_0)*eye(3);
...
while cstep<nstep
    ...
    for k=1:K
        ...
        for w=1:V
            ...
            sum_k_words=[sum_k_words; repmat([word_x,word_y,word_z],num_word,1)];
        end;
        Gaussian_means(k,:)=mean(sum_k_words);
    end;
    ...
    for m=1:M
        ...
        for n=1:N_m(m)
            ...
            sample_prob_3=zeros(1,K);  %----sapling probability of Gaussian 
            word=w;
            word_z=ceil(word/(size_box*size_box));
            residual_1=mod(word,size_box*size_box);
            word_y=max(ceil(residual_1/size_box),1);
            ...
            for z=1:K
                sigma_0_square=sigma_0^2;
                sigma_n_square=sigma_0_square*Gaussian_cov/(N_Z(z)*sigma_0_square+Gaussian_cov);
                mu_n=(mu_0/sigma_0_square+N_Z(z)*Gaussian_means(z,:)/Gaussian_cov)*sigma_n_square;
                sample_prob_3(z)=mvnpdf([word_x,word_y,word_z],mu_n,sigma_n_square+Gaussian_cov);
            end;
            sample_prob_3=sample_prob_3/sum(sample_prob_3);
            ...
            sample_prob=sample_prob.*sample_prob_3;
...
for kk=1:K  
    ...
    for m=1:M
        sum_m_k=sum(N_Z_D(m,:))+sum(alpha);
        psi(m,kk)=(N_Z_D(m,kk)+alpha(kk))/sum_m_k;
    end;
end;
